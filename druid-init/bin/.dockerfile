FROM apache/druid:35.0.1 AS base

FROM alpine

COPY --from=base /opt/druid/extensions/mysql-metadata-storage/mysql-metadata-storage-35.0.1.jar /tmp/extensions/mysql-metadata-storage/mysql-metadata-storage-35.0.1.jar
