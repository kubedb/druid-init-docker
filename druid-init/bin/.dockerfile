FROM apache/druid:36.0.0 AS base

FROM alpine

COPY --from=base /opt/druid/extensions/mysql-metadata-storage/mysql-metadata-storage-36.0.0.jar /tmp/extensions/mysql-metadata-storage/mysql-metadata-storage-36.0.0.jar
