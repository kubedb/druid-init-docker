FROM apache/druid:30.0.0 AS base

FROM alpine

COPY --from=base /opt/druid/extensions/mysql-metadata-storage/mysql-metadata-storage-30.0.0.jar /tmp/extensions/mysql-metadata-storage/mysql-metadata-storage-30.0.0.jar
