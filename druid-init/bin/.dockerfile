FROM apache/druid:28.0.0 AS base

FROM alpine

COPY --from=base /opt/druid/extensions/mysql-metadata-storage/mysql-metadata-storage-28.0.0.jar /tmp/extensions/mysql-metadata-storage/mysql-metadata-storage-28.0.0.jar
